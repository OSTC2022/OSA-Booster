...
if (!connectionString) {
  console.error(
    'SUPABASE_DB_URL 또는 SUPABASE_DB_PASSWORD가 없어 SQL을 자동 적용할 수 없습니다. ' +
      'Supabase SQL Editor에서 supabase/add-garmin-pairing-sessions.sql을 실행하세요.',
  )
  process.exit(1)
}

const sql = readFileSync(
  resolve(process.cwd(), 'supabase/add-garmin-pairing-sessions.sql'),
  'utf8',
)
const client = postgres(connectionString, { max: 1 })
try {
  await client.unsafe(sql)
  console.log('garmin pairing sessions schema applied successfully.')
} finally {
